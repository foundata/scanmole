...
| `--lineart-threshold F\|auto` | `0.5` | Black/white cutoff (fraction of full brightness) for the software lineart fallback when the device cannot scan 1-bit itself; `auto` picks a guarded per-page Otsu threshold for faint originals, `0` keeps the device's gray/color output; no effect on native-1-bit devices (see [software lineart fallback](#pipeline-lineart)) |
...
`--lineart-threshold auto` (opt-in, never the default; the GUI's "B/W (faint)" mode) targets faint originals such as thermal-paper receipts and washed-out copies, whose strokes a fixed 0.5 cut loses. It computes one guarded global Otsu threshold per page from the brightness histogram, not a spatially adaptive binarization, and every *decision* metric stays fixed-0.5: the page is first converted at 0.5 exactly as the default would, blank detection, the `page` event's `mean` and the auto-size robust bbox all come from that result, so blank backsides and paper-size choices behave identically in every thresholding mode. Only a page that survives the blank drop is adapted (a kept faint-only page via `--keep-blanks` is the recovery case), and only when the guards accept the split: both histogram classes need real weight, the class means real separation, the between-class variance must dominate (a uniform spread is rejected), the resulting ink coverage must stay document-like and must not explode relative to the fixed cut, and the threshold is clamped only afterwards to a band whose upper end (0.9) deliberately admits washed-out strokes. A rejected adaptation falls back to the fixed result; the page is never left gray by accident. An accepted one replaces the page atomically, and its reach envelope is unioned into the page's measurement so recovered strokes cannot be cropped, while the paper size still comes from the fixed bbox. The histogram pass measures ~180 to 200 ms per full-resolution A4/300 dpi page (P5 and P6), full data, no subsampling. On devices that scan native 1-bit the option has no effect, since no software conversion happens at all.
