...
No fixtures are committed yet. The current tests generate synthetic fixtures at runtime; a small field corpus may follow later, captured from purpose-built, repository-owned test sheets after privacy and provenance review. The [scanner evidence kit](../../../scripts/scanner-evidence/README.md) provides those test sheets, the capture tooling and the runbook; raw capture runs stay outside the repository, and only separately approved payloads within the budgets ever land here.
